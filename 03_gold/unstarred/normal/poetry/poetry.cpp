...
struct custom_hash {
    ...
    size_t operator()(uint64_t x) const {
        static const uint64_t FIXED_RANDOM = 0xDEADBEEFCAFEBABE; // 🍷 classic
...
// ===  Main Driver  ===
// =====================
unordered_map<int, int> count(int k, const vi &length, const vi &rhyme) {
    int n = length.size();
    vector<int> dp(k + 1);
    dp[0] = 1;
    ...
    for (int i = 0; i <= k; ++i) {
        for (int j = 0; j < n; ++j) {
            if (i + length[j] <= k) {
                dp[i + length[j]] = (dp[i + length[j]] + dp[i]) % MOD;
            }
        }
    }

    unordered_map<int, int> rhyme_count;

    for (int i = 0; i < n; ++i) {
        int len = length[i];
        int rc = rhyme[i];

        if (k - len >= 0 && dp[k - len] > 0) {
            rhyme_count[rc] = (rhyme_count[rc] + dp[k - len]) % MOD;
        }
    }
...
int main() {
    setIO("poetry"); // Change filename as needed for file I/O
    int n = r_int(), m = r_int(), k = r_int();
    vi length(n), class_(n);
    ...
    map<char, int> freq;
    for (char c : scheme) freq[c]++;

    unordered_map rhyme_count = count(k, length, class_);
    int tot = 1;
    for (auto &[letter, count] : freq) {
        long long contrib = 0;
        for (auto &[rhyme_class, count_lines] : rhyme_count) {
            if (count_lines > 0) { // ⬅️ Extra safety!
                contrib = (contrib + powmod(count_lines, count, MOD)) % MOD;
            }
        }
        tot = (1LL * tot * contrib) % MOD;
    }
