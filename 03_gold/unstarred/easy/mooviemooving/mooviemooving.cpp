...
int solve_dp(int n, int L, const vector<Movie>& movies) {
    const int FULL = 1 << n;
    ...
    for (int mask = 0; mask < FULL; ++mask) {
        ...
        for (int i = 0; i < n; ++i) if (!(mask & (1<<i))) {
            // earliest showtime that starts ≤ dp[mask]
            auto it = upper_bound(all(movies[i].showtimes), dp[mask]);
            if (it == movies[i].showtimes.begin()) continue;   // none
            --it;                                              // last ≤ t
            int start = *it;
            int end   = start + movies[i].duration;
            int nxt   = mask | (1<<i);
            dp[nxt]   = max(dp[nxt], end);
        }
    }

    int best = INT_MAX;
    for (int mask = 0; mask < FULL; ++mask)
        if (dp[mask] >= L) best = min(best, __builtin_popcount(mask));
...
int main() {
    ...
    for (int i = 0; i < n; ++i) {
        int d = r_int(), c = r_int();
        movies[i].duration = d;
        movies[i].showtimes.resize(c);
        for (int j = 0; j < c; ++j) {
            movies[i].showtimes[j] = r_int();
        }
    }
    w_int(solve_dp(n, L, movies));
    w_char('\n');
    flush();
    return 0;
}
