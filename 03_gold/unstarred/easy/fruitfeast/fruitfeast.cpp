...
int knapsack_2D(int item_num, int max_weight, const vi &weights, const vi &vals, vi &items_taken) {
    vvi dp(item_num + 1, vi(max_weight + 1, 0));

    for (int i = 1; i <= item_num; ++i) {
        for (int w = 0; w <= max_weight; ++w) {
            dp[i][w] = dp[i - 1][w];  // Not taking item i-1
            if (weights[i - 1] <= w) {
                chmax(dp[i][w], vals[i - 1] + dp[i - 1][w - weights[i - 1]]);
            }
        }
    }
...
int knapsack_1D(int item_num, int max_weight, const vi &weights, const vi &vals) {
    vi dp(max_weight + 1, 0);  // dp[w] = max value with weight limit w
    for (int i = 0; i < item_num; ++i) {
        for (int w = max_weight; w >= weights[i]; --w) {
            chmax(dp[w], vals[i] + dp[w - weights[i]]);
        }
    }
    return dp[max_weight];
}

int fill(int T, int A, int B) {
    vector<vector<bool>> dp(T + 1, vector<bool>(2));
    dp[0][0] = true;

    // First pass: do all eating + drinking transitions
    for (int i = 0; i <= T; ++i) {
        for (int used = 0; used <= 1; ++used) {
            if (!dp[i][used]) continue;

            // Try eating
            if (i + A <= T) dp[i + A][used] = true;
            if (i + B <= T) dp[i + B][used] = true;

            // Try drinking if not yet done
            if (used == 0) {
                dp[i / 2][1] = true;
            }
        }
    ...
    // Second pass: propagate post-water eating
    for (int i = 0; i <= T; ++i) {
        if (dp[i][1]) {
    ...
    // Get the max fullness possible
    for (int i = T; i >= 0; --i)
        if (dp[i][0] || dp[i][1])
            return i;

    return 0; // Bessie starves (shouldn't happen)
}
