...
int main() {
    ...
    cin.tie(0);
    setIO("lifeguards"); // Change filename as needed for file I/O
    int n = 1;
    cin >> n;
    ...
    for (int i=0; i<n; i++) {
        int s, e;
        cin >> s >> e;
        events.push_back({s, +1, i}); // start
        events.push_back({e, -1, i}); // end
    }
    ...
    for (int i = 0; i < (int)events.size() - 1; i++) {
        if (events[i].type == +1) active.insert(events[i].id);
        else active.erase(events[i].id);
        ...
        if (!active.empty()) {
            total += delta;
            if (active.size() == 1) {
                int only = *active.begin();
                solo[only] += delta;
            }
        }
    }

    cout << total - *min_element(solo.begin(), solo.end()) << "\n";
    return 0;
}
