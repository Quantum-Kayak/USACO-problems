...
int main() {
    ...
    vector<int> milk(n);
    for (int &x : milk) cin >> x;

    vector<store> stores(m);
    for (int i = 0; i < m; i++) {
        cin >> stores[i].cap >> stores[i].rate;
    }

    vector<int> rent(r);
    for (int &x : rent) cin >> x;

    sort(milk.begin(), milk.end(), greater<int>());
    sort(stores.begin(), stores.end(), cmpStore);
    sort(rent.begin(), rent.end(), greater<int>());

    // Build store sales as blocks (compressed, no per-gallon explosion)
    vector<pair<ll, ll>> storePrefix; // {total gallons so far, total profit so far}
    ll gallons = 0, profit = 0;
    for (auto &s : stores) {
        storePrefix.emplace_back(gallons + s.cap, profit + 1LL * s.cap * s.rate);
        gallons += s.cap;
        profit += 1LL * s.cap * s.rate;
    }

    auto getMilkProfit = [&](ll milkGallons) -> ll {
        if (milkGallons == 0 || storePrefix.empty()) return 0;
        
        int idx = lb(storePrefix, pll{milkGallons, 0LL});
        if (idx >= sz(storePrefix)) return storePrefix.back().s;

        ll cap = storePrefix[idx].f;
        ll val = storePrefix[idx].s;

        ll prevCap = (idx == 0 ? 0 : storePrefix[idx - 1].f);
        ll prevVal = (idx == 0 ? 0 : storePrefix[idx - 1].s);
        
        ll rate = (cap - prevCap == 0 ? 0 : (val - prevVal) / (cap - prevCap));
        return prevVal + (milkGallons - prevCap) * rate;
    };

    ll totalMilk = accumulate(milk.begin(), milk.end(), 0LL);
    ll milkUsed = totalMilk;
    ll rentProfit = 0;
    ll bestProfit = getMilkProfit(milkUsed);

    for (int i = 0; i < min(n, r); ++i) {
        milkUsed -= milk[n - 1 - i]; // renting worst cow (last in descending list)
        rentProfit += rent[i];
        ll milkProfit = getMilkProfit(milkUsed);
        bestProfit = max(bestProfit, milkProfit + rentProfit);
    }
