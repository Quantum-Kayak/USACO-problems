...
int main() {
    ...
    while (i < N) {
        ...
        bool leftZero  = (start - 1 >= 0) && (s[start - 1] == '0');
        bool rightZero = (i < N) && (s[i] == '0');
        bool isInterior = leftZero && rightZero;

        lens.push_back(len);
        interior.push_back(isInterior);
    }
    ...
    for (size_t k = 0; k < lens.size(); ++k) {
        int L = lens[k];
        int tmax = interior[k] ? (L - 1) / 2 : (L - 1);
        t_star = min(t_star, tmax);
    }

    int denom = 2 * t_star + 1;
    long long ans = 0;
    for (int L : lens) {
        ans += (L + denom - 1) / denom;
    }
