...
static inline int idx(int a, int b, int N) { return a * N + b; }
...
int main() {
    ...
    int SZ = N * N;
    vector<int> cntX(SZ, 0), cntY(SZ, 0), cntZ(SZ, 0);
    vector<unsigned char> fullX(SZ, 0), fullY(SZ, 0), fullZ(SZ, 0);
    ...
    while (Q--) {
        ...
        if (++cntX[ix] == N && !fullX[ix]) { fullX[ix] = 1; ++total; }
        if (++cntY[iy] == N && !fullY[iy]) { fullY[iy] = 1; ++total; }
        if (++cntZ[iz] == N && !fullZ[iz]) { fullZ[iz] = 1; ++total; }
